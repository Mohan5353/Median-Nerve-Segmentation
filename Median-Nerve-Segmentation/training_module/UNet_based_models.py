...
def train_UNet(args, train_loader, val_loader):
    ...
    if args.resume and checkpoint_path:
        ...
        state_dict = torch.load(args_load, map_location=device, weights_only=False)
        ...
        with tqdm(total=args.n_train, desc=f'Epoch [{epoch}/{num_epochs}]', 
            ...
            for batch in train_loader:
                ...
                pbar.set_postfix({
                    ...
                    'Train DSC': f'{train_dice/total_samples:.4f}'
                })
        ...
        with torch.no_grad():
            with tqdm(total=args.n_val, desc=f'Validation [{epoch}/{num_epochs}]',
                ...
                for val_batch in val_loader:
                    ...
                    val_outputs_binary = (val_outputs.float() > 0.5).float()

                    running_mean_val_loss = 0
                    for loss_name in args.loss:
                        running_mean_val_loss += loss_list[loss_name](val_outputs.float(), val_masks.float()).item()

                    val_loss += running_mean_val_loss * val_images.size(0)

                    running_mean_val_dice = dice_coeff(val_outputs_binary, val_masks).item() 
                    ...
                    pbar.set_postfix({
                        'Val Loss': f'{val_loss/val_samples:.4f}',
                        'Val DSC': f'{val_dice/val_samples:.4f}'
                    })

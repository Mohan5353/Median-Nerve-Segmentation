...
class FlashMultiheadAttention(nn.Module):
    ...
    Optimized for Blackwell (SM100) using FA4 if available.
    """
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None, need_weights=False):
        ...
        # and has constraints on masks.
        use_flash_lib = HAS_FLASH_ATTN and q.is_cuda and q.dtype != torch.float32
        
        # If we have masks or are using float32, we prefer PyTorch's SDPA 
        # which is more robust and still uses Flash Attention internally when possible.
        if attn_mask is not None or key_padding_mask is not None or not use_flash_lib:
            ...
            try:
                # Use specific Flash Attention library call for Blackwell/General
                if HAS_FLASH_ATTN_V4:
                    # FA4 (Blackwell optimized)
                ...
                # Fallback to SDPA if library call fails (e.g. SM capability issue)
                return self._forward_sdpa(q, k, v, N, L, E, L_k, attn_mask, key_padding_mask)
